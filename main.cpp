...
int main(){
    ...
    string commandForHomePage;
    startHomePage(commandForHomePage);


    while (commandForHomePage != "Exit") {
        switch (stoi(commandForHomePage)) {
            case 1:
                startGame(commandForHomePage);
                while (commandForHomePage != "3") {
                    switch (stoi(commandForHomePage)) {
                        case 1:
                            hpInputUserName(userName);
                            hpNewGame(commandForHomePage);
                            while (commandForHomePage != "3") {
                                switch (stoi(commandForHomePage)) {
                                    case 1:
                                        hpInputUserName(userName);
                                        // start new game with user name
                                        break;
                                    case 2:
                                        hpDifficulty(commandForHomePage);
                                        // start new game with selected difficulty
                                        while(commandForHomePage != "5"){
                                            switch (stoi(commandForHomePage)){
                                                case 1:
                                                    difficulty = "easy";
                                                    break;
